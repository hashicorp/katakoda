...
ssh root@host01 "mkdir -p ~/log && nohup sh -c \"consul agent -dev -advertise_addr=$(ip route get 1 | awk '{print $NF;exit}') >~/log/consul.log 2>&1 & nohup nomad agent -dev -bind=0.0.0.0 >~/log/nomad.log 2>&1 &\" &"
